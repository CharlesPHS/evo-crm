  ...
  module V1
      class InboxesController < Api::V1::BaseController
        ...
        def create_channel
          ...
          channel_params = permitted_params(channel_type_from_params::EDITABLE_ATTRS)[:channel].except(:type)

          if channel_params[:provider] == 'evolution_go'
            Rails.logger.info "Creating Evolution Go channel with params: #{channel_params.inspect}"
            channel_params = inject_evolution_go_global_config(channel_params)
          end

          account_channels_method.create!(channel_params)
        end

        def inject_evolution_go_global_config(channel_params)
          provider_config = (channel_params[:provider_config] || {}).dup

          if provider_config['api_url'].blank?
            global_api_url = GlobalConfigService.load('EVOLUTION_GO_API_URL', '').to_s.strip
            provider_config['api_url'] = global_api_url if global_api_url.present?
          end

          if provider_config['admin_token'].blank?
            global_admin_token = GlobalConfigService.load('EVOLUTION_GO_ADMIN_SECRET', '').to_s.strip
            provider_config['admin_token'] = global_admin_token if global_admin_token.present?
          end

          channel_params.merge(provider_config: provider_config)
        end
